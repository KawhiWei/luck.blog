+ # 自动内存管理和CLR的理解
  ...
  + ## 垃圾回收
    ...
    + ## 前台和后台的垃圾回收区别
      ...
      + ### 前台垃圾回收
        发生前台垃圾回收的时候所有的托管线程会处于挂起阶段，也就是说这个时候是GC线程执行的阶段，其他线程不处理任务。另外就是前台的垃圾回收也是由后台垃圾回收线程去执行的，它是这么做的<font color=red>**如果后台垃圾回收的线程在检测是否有前台发起垃圾回收指令，如果收到了就挂起后台的垃圾回收的线程，执行前台的垃圾回收的线程，大白话说法就是二者只能选其一。**</font>
      + 
    + 
  + ## GC执行的流程？
